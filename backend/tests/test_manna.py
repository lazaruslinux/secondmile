...
def pending(db_session, user_id: int) -> int:
    db_session.expire_all()
    row = db_session.get(models.UserProgress, user_id)
    return 0 if row is None else row.manna_pending
...
def test_a_credited_workout_puts_its_calories_in_the_pending_pile(
...
def test_a_rebuild_recomputes_the_pending_manna_from_what_survives(
    signed_in, db_session, member
):
    ...
    assert pending(db_session, member.id) == 950

    response = signed_in.delete(f"/api/workouts/{doomed.id}")
    assert response.status_code == 204, response.text
    assert pending(db_session, member.id) == 650

    # And a restore puts back exactly what the deletion took, which is what
    # makes the pile a derivation rather than a running tally.
    response = signed_in.post(f"/api/workouts/{doomed.id}/restore")
    assert response.status_code == 200, response.text
    assert pending(db_session, member.id) == 950


def test_recompute_lands_on_the_same_pile_it_started_with(signed_in, db_session, member):
    ...
    assert pending(db_session, member.id) == 1105
...
def test_a_rebuild_reads_no_steps_at_all(signed_in, ingest_token, db_session, member):
    ...
    progress.recompute(db_session, member.id)
    assert pending(db_session, member.id) == 400
...
def test_a_feed_card_carries_no_manna(signed_in, db_session, member):
    ...
    through nothing else, and giving is not built yet."""
    friend, mate = sign_in(db_session, "mate")
    befriend(db_session, member, friend)
    log_workout(db_session, member.id, "run", 5.0, kcal=650)
...
def test_the_letter_covers_the_same_window_the_miles_do(signed_in, db_session, member):
    ...
    since, and never the pile the account is sitting on."""
    log_workout(db_session, member.id, "run", 5.0, kcal=650)
...
def test_nothing_counts_down(signed_in, db_session, member):
    """No spoilage in this round: the 7-day clock is a gathered-goods rule and
    there is nothing to gather yet. A pile read a week later is the same pile,
